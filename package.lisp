...
;;          ;: FIXME: gray-streams is not a part of ANSI Common Lisp.
;;          ;;
;;          ;; This "patch" code results in changes to the
;;          ;; Gray Sreams implementation, in some implementations
;;          ;; (e.g. CLISP) and may result in significant incompatibility
;;          ;; with the Gray Streams implementation, in some other
;;          ;; implementations (e.g. ECL)
;;          ;;
;;          ;; In regards to methodology, it should not be necessary to
;;          ;; export a full symbbol table, if only to ensure
;;          ;; compatibility of a few "overlapping" symbols --
;;          ;; e.g. OUTPUT-STREAM-P and INPUT-STREAM-P, such that both
;;          ;; Gray Streams[1] and CLIM define.[2]
;;          ;;
;;          ;; This implementation might extend to utilize Trivial Gray
;;          ;; Streams[3] to ensure compatibility of Gray Streams
;;          ;; symbols, CLIM, and ANSI Common Lisp
;;          ;;
;;          ;; [1] http://franz.com/support/documentation/6.0/ansicl/dictentr/input-st.htm
;;          ;; [2] http://bauhh.dyndns.org:8000/clim-spec/D-1.html
;;          ;; [3] http://common-lisp.net/project/trivial-gray-streams/
...
;;            #:in-package #:incf #:initialize-instance #:inline #:inspect #:integer
...
;;            #-ECL #:input-stream-p #-ECL #:output-stream-p
;;            ))
;;         (packages
;;          #+gcl    '(:lisp :pcl)
;;          #-(or gcl) '(:common-lisp))
